    ...
    <div className="bg-app min-h-screen">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        <header className="mb-6">
          <h1 className="text-3xl font-semibold tracking-tight text-slate-900">Invite team members</h1>
                  ...
                  <li key={i.id} className="rounded-lg border border-slate-200 bg-white p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="font-medium text-slate-900 truncate">{i.full_name}</div>
...
function InviteStatus({ invite }: { invite: { used_at: string | null; expires_at: string } }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[11px] font-medium shrink-0 ${map.bg} ${map.text}`}>
